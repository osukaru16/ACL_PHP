    ...
    if (isset($_GET['action']) == 'role') { 
        ...
        <form action="roles.php" method="post">
            ...
            /*foreach ($aPerms as $k => $v)
            {
                ...
                //if (isset($rPerms[$v['Key']]['value'])  && $_GET['roleID'] != '') { echo " checked=\"checked\""; }

                echo " /></td>";
                echo "<td><input type=\"radio\" name=\"perm_" . $v['ID'] . "\" id=\"perm_" . $v['ID'] . "_0\" value=\"0\"";
                if ($rPerms[$v['Key']]['value'] != true && isset($_GET['roleID']) != '') { echo " checked=\"checked\""; }
                echo " /></td>";
				echo "<td><input type=\"radio\" name=\"perm_" . $v['ID'] . "\" id=\"perm_" . $v['ID'] . "_X\" value=\"X\"";
                if (isset($_GET['roleID']) == '' || !array_key_exists($v['Key'],$rPerms)) { echo " checked=\"checked\""; }
                echo " /></td>";
                echo "</tr>";
            }*/
            foreach ($aPerms as $k => $v)
